...
#pragma mark - Class's static private methods
+ (__autoreleasing FwiJson *)_JsonFromObject:(id)object {
	/* Condition validation */
	if (!object) return [FwiJson null];

	if      ([object isMemberOfClass:[FwiJson class]]   ) return object;
	else if ([object isMemberOfClass:[NSNull class]]    ) return [FwiJson null];
    else if ([object isMemberOfClass:[NSData class]]    ) return [FwiJson stringWithString:[(NSData *)object toString]];
	else if ([object isMemberOfClass:[NSString class]]  ) return [FwiJson stringWithString:(NSString *)object];
	else if ([object isMemberOfClass:[NSURL class]]     ) return [FwiJson stringWithString:[(NSURL *)object absoluteString]];
        ...
        __autoreleasing NSArray *array = (NSArray *) object;
        __autoreleasing FwiJson *o = [FwiJson array];
        
        for (id obj in array) {
            [o addJsons:[self _JsonFromObject:obj], nil];
        }
		return o;
	}
    else if ([object isKindOfClass:[NSDictionary class]]) {
        __block NSMutableDictionary *dictionary = [NSMutableDictionary dictionaryWithCapacity:((NSDictionary *)object).count];
        [((NSDictionary *) object) enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL *stop) {
            dictionary[key] = [self _JsonFromObject:value];
        }];
        
        __autoreleasing FwiJson *o = [FwiJson object];
...
+ (__autoreleasing id)_objectFromJson:(FwiJson *)json {
	switch ([json jsonType]) {
		case kJson_Boolean:
		case kJson_Number : return [json getNumber];
		case kJson_String : return [json getString];
		case kJson_Array  : {
            __block NSMutableArray *a = FwiAutoRelease([[NSMutableArray alloc] initWithCapacity:[json count]]);
            [[json array] enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                [a insertObject:[FwiJson _objectFromJson:obj] atIndex:idx];
            }];
            return a;
        }
		case kJson_Object : {
            __block NSMutableDictionary *d = FwiAutoRelease([[NSMutableDictionary alloc] initWithCapacity:[json count]]);
            [[json dictionary] enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                d[key] = [FwiJson _objectFromJson:obj];
            }];
            return d;
        }
		case kJson_Null   :
...
+ (void)_array:(NSMutableArray *)array arg:(id)arg args:(va_list)args {
    ...
    [array addObject:[FwiJson _JsonFromObject:arg]];
...
+ (void)_dictionary:(NSMutableDictionary *)dictionary arg:(id)arg args:(va_list)args {
    ...
    while ((arg = va_arg(args, id))) {
        if (isKey) {
            dictionary[k] = [FwiJson _JsonFromObject:arg];
            isKey = NO;
        }
        else {
            if ([arg isKindOfClass:[NSString class]]) k = arg;
...
- (NSString *)description {
    switch (_jsonType) {
		case kJson_String : return _string;
		case kJson_Boolean: return [_number boolValue] == NO ? @"False" : @"True";
		case kJson_Number : return [_number description];
		case kJson_Array  :
		case kJson_Object : {
...
+ (__autoreleasing FwiJson *)array:(id)json, ... NS_REQUIRES_NIL_TERMINATION {
    ...
    if (json) {
        va_list args;
        va_start(args, json);
        [FwiJson _array:[o array] arg:json args:args];
        va_end(args);
    }
	return o;
}
+ (__autoreleasing FwiJson *)arrayWithArray:(NSArray *)objects {
	return [FwiJson _JsonFromObject:objects];
}
...
+ (__autoreleasing FwiJson *)object:(id)key, ... NS_REQUIRES_NIL_TERMINATION {
    ...
    if (key) {
        va_list args;
        va_start(args, key);
        [FwiJson _dictionary:[o dictionary] arg:key args:args];
        va_end(args);
    }
	return o;
}
...
- (void)setJson:(id)json {
    ...
    [_array addObject:[FwiJson _JsonFromObject:json]];
}
...
- (void)setKey:(id)key andJson:(id)json {
    ...
    _objects[key] = [FwiJson _JsonFromObject:json];
}
