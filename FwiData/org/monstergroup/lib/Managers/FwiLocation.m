...
CLLocationCoordinate2D (^FwiCalculateCoordinateWithDistance)(double bearing, CLLocationCoordinate2D location, double distance) = ^(double bearing, CLLocationCoordinate2D location, double distance) {
    ...
    double lat1 = FwiConvertToRadian(location.latitude);
    double lng1 = FwiConvertToRadian(location.longitude);
    double lat2 = asin(sin(lat1) * cos(distance) + cos(lat1) * sin(distance) * cos(bearing));
    double lng2 = lng1 + atan2(sin(bearing) * sin(distance) * cos(lat1), cos(distance) - sin(lat1) * sin(lat2));
...
- (id)copyWithZone:(NSZone *)zone {
    return [FwiLocation sharedInstance];
}
- (id)retain {
    return [FwiLocation sharedInstance];
}
